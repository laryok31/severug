...
class User(db.Model):
    ...
    show_profile = db.Column(db.Boolean, default=True)  # Показывать ли профиль
    show_phone = db.Column(db.Boolean, default=False)   # Показывать ли телефон
    show_email = db.Column(db.Boolean, default=False)   # Показывать ли email
    show_rating = db.Column(db.Boolean, default=True)   # Показывать ли рейтинг
    show_history = db.Column(db.Boolean, default=False) # Показывать ли историю поездок
    bio = db.Column(db.String(500))                      # О себе
    avatar_url = db.Column(db.String(500))               # Ссылка на аватар
    ...
    show_history = db.Column(db.Boolean, default=False)
...
class Referral(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    referrer_id = db.Column(db.Integer, db.ForeignKey('user.id'))  # Кто пригласил
    friend_id = db.Column(db.Integer, db.ForeignKey('user.id'))    # Кто пришел
    created_at = db.Column(db.DateTime, default=datetime.now)
...
class GeneralChat(db.Model):
    ...
    # ВАЖНО: используем уникальное имя и явно указываем foreign_keys
    author = db.relationship('User', foreign_keys=[user_id], backref='city_chat_posts')